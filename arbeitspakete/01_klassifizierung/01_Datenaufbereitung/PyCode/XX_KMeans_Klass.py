...
file_path = "250331_Kmeans_Klass\\KMeans_input\\02_PW_KOO_RGB_HSV_normiert_0_1.txt"  # Falls der Name anders ist, bitte anpassen
...
with tqdm(total=100, desc="Clustering") as pbar:
    ...
    df["Color Cluster"] = kmeans.fit_predict(df[["Hue (°)", "Saturation (%)", "Value (%)", "Z scan dir"]])
    pbar.update(100)

# Neue Datei speichern (mit Farbklassen, ohne Header)
output_folder = f"KMeans_Clustered_Files_GS_{num_clusters}"
