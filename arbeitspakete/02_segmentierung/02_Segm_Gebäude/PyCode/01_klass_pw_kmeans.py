...
file_path = r"arbeitspakete\02_segmentierung\02_Segm_Gebäude\input\P3A1_Gebaeude.txt"  # Falls der Name anders ist, bitte anpassen
...
num_clusters = 100 # Kann auf kleine Werte (3-5) = mehr generalisierte Klassen, grosse Werte mehre clusters einer Klasse
code = "XY" # fit Funktion auf diese Variablen
...
output_folder = fr"arbeitspakete\02_segmentierung\02_Segm_Gebäude\output\KMeans_{code}_Clustered_Files_{num_clusters}"
...
for cluster_id in tqdm(df["Color Cluster"].unique(), desc="Speichern der Cluster"):
    ...
    # output_file = os.path.join(output_folder, f"P3A1_Gebaeude_{code}_Klasse_{int(cluster_id)}_kmeans.txt")
...
# Punktwolke anzeigen
o3d.visualization.draw_geometries([pcd], window_name="Segmentierte Punktwolke mit KMeans")
