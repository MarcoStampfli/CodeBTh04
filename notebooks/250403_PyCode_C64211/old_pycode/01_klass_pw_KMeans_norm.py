...
file_path = "250327_Normalisieren\output\PW_P3A1_normalisiert.txt"  # Falls der Name anders ist, bitte anpassen
...
with tqdm(total=100, desc="Clustering") as pbar:
    ...
    df["Color Cluster"] = kmeans.fit_predict(df[["Hue (0-1)", "Saturation (0-1)", "Value (0-1)",
                                                 "Z scan dir"]])
    pbar.update(100)

# Neue Datei speichern (mit Farbklassen, ohne Header)
output_folder = f"KMeans_Clustered_Files_{num_clusters}_fit_HSV_Z"
