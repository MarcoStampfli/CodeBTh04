...
for spalte, titel in achsen.items():
    ...
    plt.xlim(0, 100)
    #plt.xticks([-1, 0, 1])
    plt.title(f"Dichteverteilung {titel} je Klasse")
    plt.xlabel(f"{titel} (%)")
    plt.ylabel("Dichte")
    ax.legend(title="Klassen", loc="upper right", labels=alle_klassen)
...
for spalte, titel in achsen2.items():
    plt.figure(figsize=(10, 6))
    ax = sns.kdeplot(
        data=df_gesamt,
        x=spalte,
    ...
    plt.xlim(-10, 370)
    plt.xticks([0, 120, 240, 360],labels=["Rot 0°", "Grün 120°", "Blau 240°", "Rot 360°"])
    plt.title(f"Dichteverteilung {titel} je Klasse")
    plt.xlabel("Farbton (0 bis 360°)")
    plt.ylabel("Dichte")
